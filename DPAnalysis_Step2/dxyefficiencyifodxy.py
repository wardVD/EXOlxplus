...
def function (lamb,ctau,phot):
    ...
    biglist[1] = loop(vecfilessig, 2,phot)
    biglist[2] = loop(vecfilessig, 4,phot)
    biglist[3] = loop(vecfilessig, 6,phot)
    biglist[4] = loop(vecfilessig, 8,phot)
    biglist[5] = loop(vecfilessig, 10,phot)
    biglist[6] = loop(vecfilessig, 12,phot)
    biglist[7] = loop(vecfilessig, 14,phot)
    biglist[8] = loop(vecfilessig, 16,phot)
    biglist[9] = loop(vecfilessig, 18,phot)
    biglist[10] = loop(vecfilessig, 20,phot)
    biglist[11] = loop(vecfilessig, 22,phot)
    biglist[12] = loop(vecfilessig, 24,phot)
    ...
    EfficiencyHist = TH1D("Efficiencies","",13,0,26)
...
def main():
    ...
    CTAU10.GetYaxis().SetRangeUser(0.,1.5)
    ...
    #canvas.SaveAs("./dxyefficiencyifodxyL180.png")
    canvas.SaveAs("./dxyefficiencyifodxyL160.png")
